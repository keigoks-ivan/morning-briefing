...
def run_evidence_layer(data: dict, rss_items: list[dict] | None, watchlist: list[dict] | None,
    ...
    if jev is None:
        # 請求上限＝候選數＋2 備用。2026-09-24：投資想法／查核點步驟改成程式挑候選、Claude Opus
        #（訂閱 CLI）同批判斷，不再問 Jev，公式裡不用再留想法步驟的名額（舊版 IDEA_ITEM_BUDGET
        # 已退場，見 ideas_layer.py）。
        jev = JevClient(api_key=get_api_key(), cache=load_jev_cache(today, data_dir, fetch),
                        max_requests=MAX_CANDIDATES + 2)
    ...
    # ④ 投資想法／查核點（2026-09-23 新增，2026-09-24 改版不再問 Jev，見 ideas_layer.py）：獨立
    # 一層，包自己的 try/except，失敗只讓這一步標 unavailable，不影響事件判斷層其餘輸出。刻意跑
    # 在 items 全部組好之後（需要每則的 classification／routes／companies）；idea_hits.json 的
    # 合併規則見 ideas_layer._merge_hits。
    ...
    try:
        ...
        ideas_result = run_ideas_step(items, cand_by_id, today, ideas=ideas, fetch=fetch,
                                      matcher=matcher, rss_items=rss_items, ledger=ledger,
                                      curated_cards=curated_pool, sitemap_items=sitemap_pool,
                                      full_text_fetch=idea_full_text_fetch, judge_call=idea_judge_call)
        ...
        ideas_result = {"status": "unavailable", "reason": f"ideas step error ({type(e).__name__}: {e})",
                        "ideas_count": 0, "matched_pairs": 0, "kept": 0, "dropped_by_cap": 0,
                        "catalog": {}, "hits": None, "wide_scan": {}, "wide_pairs": []}
    ...
    result = {
        ...
        "ideas": {"status": ideas_result["status"], "reason": ideas_result.get("reason", ""),
                 ...
                 "kept": ideas_result.get("kept", 0),
                 "dropped_by_cap": ideas_result.get("dropped_by_cap", 0),
                 "catalog": ideas_result.get("catalog") or {},
                 # 2026-09-24：早報外掃描統計（見 ideas_layer._wide_scan）＋早報外每對判斷結果
                 # （含 unrelated，只給週度校準用，不對外渲染；早報外的非 unrelated 判斷已經
                 # 另外進 idea_hits.json，見 ideas_layer.run_ideas_step）
...
def unavailable_result(today: str, reason: str) -> dict:
    ...
    return {"schema": SCHEMA, "date": today, "generated_at": _now_iso(), "model": MODEL,
            ...
            "ideas": {"status": "unavailable", "reason": reason, "ideas_count": 0, "matched_pairs": 0,
                     "kept": 0, "dropped_by_cap": 0, "catalog": {}, "wide_scan": {}, "wide_pairs": []},
            "idea_hits": None,
