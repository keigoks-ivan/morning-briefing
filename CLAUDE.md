...
**GDELT 候選（2026-09-23 新增）**：早報自己的候選只來自約 60 個 RSS 來源，個股專屬新聞常常漏接。`gdelt_source.py` 另外用公司名組 OR 查詢查 GDELT DOC 2.0，公司名優先序是 DD 已有報告／系統持倉在前，研究主題成員在後；`sourcelang:english`，一次最多 20 個請求，限流一次／5 秒（超過回 HTTP 429＋純文字訊息，程式判斷後等 5 秒重試一次，再不行就放棄那一題；連續 3 題都被擋就整段收手，記 `stopped_reason: rate_limited_3_in_a_row`）。查詢字元上限（`MAX_QUERY_CHARS`）先用保守值 1200：2026-09-23 本機測太多次，IP 被封，，隔了 20～90 秒還是 429，沒能實測出 GDELT 真正的上限，之後常態性因為太長被拒再收窄。標題要點到一家研究公司、而且有帶單位的數字或主題／環節辨識詞才留；跟既有新聞卡或 RSS 標題近似的丟掉，黑名單網域（`source_registry.is_blacklisted`）丟掉；每天最多留 10 則，排序先看公司在不在 DD／持倉，再看有沒有數字，最後比新舊。候選的 `block` 叫 `gdelt`，`BLOCK_PRIORITY` 裡優先序全早報最低；`MAX_CANDIDATES` 24→30，留 8 個名額給 GDELT（配合 `JevClient` 預設每次執行 30 個請求的上限，讓保留的名額都問得到 Jev；估算單則輸入 token 沒有明顯變化，400,000 上限還很有餘裕）。`run_evidence_layer` 新增 `gdelt_fetch` 參數，預設 `None`（不查、不連網，測試安全）；`main.py` 傳 `gdelt_fetch=fetch_gdelt_candidates` 才真的查。GDELT 掛掉、逾時、被限流都包一層 try/except，不影響早報，記在 `quality["gdelt"]`（`requests_sent`／`ok`／`rate_limited`／`articles_seen`／`kept`／`slots_used`）與 `source_quality_*.json`。news 頁每則 GDELT 候選旁邊有「Found via GDELT (not in the briefing's feeds)」標籤（`html_template._ev_item`）。**2026-09-24 起這 8 個名額改成跟下面的 sitemap 候選共用**，不是各自獨立 8 個，見下一段。

**GDELT 在 GitHub Actions 上被限流、加 sitemap 候選（2026-09-24 新增）**：2026-09-24 排程實測 GDELT 連續 3 個查詢都被擋（`stopped_reason: rate_limited_3_in_a_row`），`kept: 0`——雲端 IP 比本機更容易被 GDELT 的限流機制盯上。既有的重試機制沒辦法解決「IP 本身被列管」這種問題，另開一條不靠 GDELT 搜尋索引的候選管道：`sitemap_source.py` 直接讀新聞網站自己發布的 Google News 相容 sitemap（`<news:title>`／`<news:publication_date>`），來源清單在 `data/news_sitemaps.json`（可編輯，不用改程式，目前 7 個：CNBC、Reuters、TheElec、Korea Herald、Nikkei Asia、Tom's Hardware、TrendForce）。三種 `type`：`news_sitemap`（直接有 news: 標籤）、`sitemap_index`（先抓索引頁，取最新 `max_children` 個子 sitemap 當 news_sitemap 解析——Nikkei Asia 按日期分檔，`max_children=2` 剛好對到 48 小時窗）、`site_sitemap`（沒有 news: 標籤，只有 `lastmod`，靠 `url_filter` 正則篩路徑，標題用網址 slug 猜，標 `title_from_slug: true`——TrendForce 的 slug 是日期＋流水號，猜不出有意義的標題）。每個來源最多重試一次（不分析回應內容，固定等 2 秒），被擋（403／429）或解析失敗就那個來源標對應狀態＋0 則，不影響其他來源，也不再多試第三次。

篩選比 GDELT 寬：標題點到研究公司「或」點到研究主題／環節辨識詞就留（GDELT 是「公司且數字或主題」），因為 sitemap 來源本身是精選媒體、不是搜尋索引；跟既有標題近似的丟掉、黑名單網域丟掉，跟 GDELT 共用 `evidence_routing._keyword_hits`／`news_fetcher._near_same_title`／`source_registry.is_blacklisted` 同一套規則。候選 `block` 叫 `sitemap`，優先序跟 gdelt 一樣是 0.2。**GDELT／sitemap 共用 `GDELT_MAX_SLOTS`（8）名額，不是各自 8 個**：`evidence_layer._merge_reserved_candidates` 把兩邊候選合併，依「優先公司命中 → 有數字 → 新舊」排序取前 `slots` 個（GDELT 沒查到就整批由 sitemap 補，兩邊都有就交錯排序，不是先放完一個來源）；跨來源近似標題也會互相去重，只留分數較高的那則。兩份 quality 各自多記 `slots_used`（這次合併實際用掉幾個名額）。`run_evidence_layer` 新增 `sitemap_fetch` 參數，預設 `None`；`main.py` 傳 `sitemap_fetch=fetch_sitemap_candidates` 才真的抓。任何一步出錯都包 try/except，不影響早報，記在 `quality["sitemap"]`（`sources`：每個來源的 `status`／`items_seen`／`items_recent`；`items_seen_total`／`items_recent_total`／`matched`／`kept`／`kept_titles`／`pool_size`／`slots_used`）。news 頁每則 sitemap 候選旁邊有「Found via sitemap (not in the briefing's feeds)」標籤，來源品質行多一句 Sitemap 摘要，抓不到的來源也會列出來（跟官方來源、RSS feed 同一套待遇，見 `html_template._ev_quality_line`）。

**早報外掃描也吃得到（`ideas_layer._wide_scan` 新增 `sitemap_items` 參數）**：跟 `curated_cards`（既有新聞卡）並列，但 sitemap 項目已經是 RSS 條目形狀（`sitemap_source.to_pool_items()`，`link` 是真實網址，不用像 curated_cards 那樣轉形狀、也不會因此漏接網址比對）；ALL 近期項目（不只公司命中的）都送進去，讓查核點關鍵詞比對自己去配，既有把關（過舊、ledger 已知數字、idea_hits 歷史重複、跟早報候選標題近似）不變。已經變成 evidence 候選的 sitemap 項目網址會被既有的 `used_urls` 比對自然排除，不用另外濾一次。`wide_scan` 統計多一個 `sitemap_pool_size`。

**沒收的來源**：`money.udn.com`（經濟日報）有現成的 gnews sitemap 且即時更新，但 robots.txt 明文寫「Any other uses are not permitted...for large language models (LLMs)...」且直接列名 `Disallow: Claude`／`ClaudeBot`——這不是普通的爬蟲節流，是站方對 LLM／AI 用途的明文禁止，沒有收進來；DigiTimes 英文站只有 RSS（`daily.xml`），`sitemap.xml`／`sitemap_news.xml` 等路徑實測都是 200 但回應是 SPA 的 soft-404 頁面，不是真的 sitemap；`ctee.com.tw`（工商時報）的 sitemap 在本機測試被 Cloudflare WAF 擋掉回 403；`cnyes`（鉅亨）的 news sitemap（`desktop-news.xml.gz`）快照裡最新一筆是 2021 年，不是即時更新的清單。四個都不收，理由寫在 `data/news_sitemaps.json` 的 `_comment`。
