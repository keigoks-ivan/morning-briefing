...
- 素材兩層：① `RSS_FEEDS`（news_fetcher.py）多來源 feed 並行抓，抓取端先以 `source_registry.py` 做來源正規化與白名單硬驗證，再以 URL／近似標題去重並 round-robin 保留各來源覆蓋，總上限 310；② `PERPLEXITY_QUERIES` 共 15 題，保留「需跨來源整理」的主題。深挖不再先跑 meta-query，固定並行 2 題；MOVE 另算 1 次。WSJ／Nikkei 官方 RSS 已停更，不要加回。
